...
public class AudioManager : MonoBehaviour
{
    ...
    private void Start()
    {
        originalVol = musicPlayer.volume;
    ...
    {
        if (clip == null || clip == curMusic) return;
    ...
    IEnumerator PlayMusicAsync(AudioClip clip, bool loop, bool fade)
    {
        ...
        if (fade)
            yield return musicPlayer.DOFade(originalVol, fadeDuration).WaitForCompletion();
    }
    ...
    IEnumerator UnPauseMusic(float delay)
    {
        ...
        musicPlayer.UnPause();
        musicPlayer.DOFade(originalVol, fadeDuration);
    }
}

public enum AudioId { UISelect, Hit, Faint, ExpGain, ItemGain }
